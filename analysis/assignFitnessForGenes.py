...
def getSgRNAByPosAndStrand (left, right, strand):
    ...
    cursorT.execute("select s.pos, s.seq,p.pval,p.logFC from sgRNA s LEFT JOIN anaerobic_PVALS p on s.seq=p.seq"+
...
for (gene_id,name,left,right,strand) in cursor:
    ...
    cursorN.execute('update GENE set extreme=%s, mean=%s,median=%s where gene_id=%s',(val,mean,median,gene_id))